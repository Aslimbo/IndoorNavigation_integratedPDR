...
import static com.example.locate.Constants.readCharacteristicUUID;
import static com.example.locate.Constants.serviceUUID;
import static com.example.locate.Constants.writeCharacteristicUUID;
...
class SerialSocket extends BluetoothGattCallback {
    ...
    SerialSocket(Context context, BluetoothDevice device) {
        ...
        Log.d(TAG, "connected device test");
...
//                onSerialConnectError(new IOException(context.getString(R.string.pairing_request)));
    ...
    private void connectCharacteristics1(BluetoothGatt gatt) {
        ...
        for (BluetoothGattService gattService : gatt.getServices()) {
            ...
            if (gattService.getUuid().equals(UUID.fromString(serviceUUID)))
    ...
    private void receive(ArrayDeque<byte[]> datas) {
        ...
        if(parts.length==4){
            ...
            if (BLEserialManager.apFilters.containsKey(macAddress)) {
                KalmanFilter filter = BLEserialManager.apFilters.get(macAddress);
...
//                                Log.d("filtered_rssi_" + macAddress, String.valueOf(filteredValue));
                    ...
                    BLEserialManager.apRawRSSI.put(macAddress, Double.parseDouble(rssi));
                    BLEserialManager.apFilteredRSSI.put(macAddress,filteredValue);

                    double[] RawValuesArray = BLEserialManager.getFilteredRSSIValuesArray(BLEserialManager.apRawRSSI);
...
//                                Log.d("RawValuesArray", Arrays.toString(RawValuesArray));
                    ...
                    new Handler(Looper.getMainLooper()).post(() -> {
                        ...
                        BLEserialManager.tvScanResult.setText(resultText);
                    });
    ...
    private class CustomDelegate extends DeviceDelegate {
        @Override
        boolean connectCharacteristics(BluetoothGattService gattService) {
            Log.d(TAG, "service Custom Profile");
            readCharacteristic = gattService.getCharacteristic(UUID.fromString(readCharacteristicUUID));
            writeCharacteristic = gattService.getCharacteristic(UUID.fromString(writeCharacteristicUUID));
            return true;
        }
    }
